...
namespace MSS6x_Flasher
{
    class Checksums_Signatures
    {
        ...
        public byte[] CorrectParameterChecksums(byte[] DataToFlash)
        {
            ...
            int numberOfSegments = BitConverter.ToInt32(DataToFlash.Skip(0x1C0).Take(4).Reverse().ToArray(), 0);
            int checksumStart = 0;
            int checksumEnd = 0;
            uint crc = 0xFFFFFFFF;
            int maxAddress = 0x7FFFB; //This is hardcoded in the DME
            int MemSubtract = 0x70000;

            for (int i = 0; i < numberOfSegments; ++i)
            {
                checksumStart = BitConverter.ToInt32(DataToFlash.Skip(0x1C4 + (8 * i)).Take(4).Reverse().ToArray(), 0) - MemSubtract;

                checksumEnd = BitConverter.ToInt32(DataToFlash.Skip(0x1C8 + (8 * i)).Take(4).Reverse().ToArray(), 0) - MemSubtract;
                ...
                crc = CalculateCRC32(DataToFlash.Skip(checksumStart).Take((checksumEnd - checksumStart) + 1).ToArray(), crc);
            }
            crc = ~crc;
        ...
        public byte[] CorrectProgramChecksums(byte[] DataToFlash)
        {
            ...
            int numberOfSegments = BitConverter.ToInt32(DataToFlash.Skip(0x101C0).Take(4).Reverse().ToArray(), 0);
            int checksumStart = 0;
            int checksumEnd = 0;
            uint crc = 0xFFFFFFFF;
            int maxAddress = BitConverter.ToInt32(DataToFlash.Skip(0x1001C).Take(4).Reverse().ToArray(), 0) + 3;
            int MemSubtract = 0x380000;

            for (int i = 0; i < numberOfSegments; ++i)
            {
                checksumStart = BitConverter.ToInt32(DataToFlash.Skip(0x101C4 + (8 * i)).Take(4).Reverse().ToArray(), 0);
                checksumEnd = BitConverter.ToInt32(DataToFlash.Skip(0x101C8 + (8 * i)).Take(4).Reverse().ToArray(), 0);
        ...
        public bool IsParamSignatureValid(byte[] binary)
        {
            ...
            try
            {
                CalculatedMD5_Injection = CalculateParamMD5(binary.Take(0x10000).ToArray());
            }
            catch
            {
                return false;
            ...
            try
            {
                CalculatedMD5_Ignition = CalculateParamMD5(binary.Skip(0x10000).Take(0x10000).ToArray());
            }
            catch
            {
                return false;
        ...
        public bool IsProgramSignatureValid(byte[] binary)
        {
            ...
            try
            {
                CalculatedMD5_Injection = CalculateProgramMD5(binary.Take(0x280000).ToArray());
            }
            catch
            {
                return false;
            ...
            try
            {
                CalculatedMD5_Ignition = CalculateProgramMD5(binary.Skip(0x280000).Take(0x280000).ToArray());
            }
            catch
            {
                return false;
        ...
        private byte[] CalculateParamMD5(byte[] binary)
        {
            int numberOfSegments = BitConverter.ToInt32(binary.Skip(0x1C0).Take(4).Reverse().ToArray(), 0);
            int segment_start = 0;
            int segment_end = 0;
            int MemSubtract = 0x70000;
            ...
            for (int i = 0; i < numberOfSegments; ++i)
            {
                segment_start = BitConverter.ToInt32(binary.Skip(0x1C4 + (8 * i)).Take(4).Reverse().ToArray(), 0) - MemSubtract;
                segment_end = BitConverter.ToInt32(binary.Skip(0x1C8 + (8 * i)).Take(4).Reverse().ToArray(), 0) - MemSubtract;
                //Console.WriteLine(checksumStart.ToString("x"));
                //Console.WriteLine(checksumEnd.ToString("x"));

                toHash = toHash.Concat(binary.Skip(segment_start).Take((segment_end - segment_start) + 1)).ToArray();
            }
        ...
        private byte[] CalculateProgramMD5(byte[] binary)
        {
            int numberOfSegments = BitConverter.ToInt32(binary.Skip(0x101C0).Take(4).Reverse().ToArray(), 0);
            int segment_start = 0;
            int segment_end = 0;
            int MemSubtract = 0x380000;
            ...
            for (int i = 0; i < numberOfSegments; ++i)
            {
                segment_start = BitConverter.ToInt32(binary.Skip(0x101C4 + (8 * i)).Take(4).Reverse().ToArray(), 0);

                segment_end = BitConverter.ToInt32(binary.Skip(0x101C8 + (8 * i)).Take(4).Reverse().ToArray(), 0);
